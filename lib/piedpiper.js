...
class PiedPiperCompressor {
    constructor() {
        ...
        this.MAX_CHAIN_LENGTH = 256;     // Limit hash chain length
    ...
    // Advanced PIPER match finding with optimized hash chains
    findBestMatch(data, pos, hashTable, hashChains) {
        ...
        for (let i = chain.length - 1; i >= Math.max(0, chain.length - searchLimit); i--) {
            const matchPos = chain[i];

            if (matchPos < windowStart || matchPos >= pos) continue;

            // Quick check: verify first and last bytes match
            ...
            if (matchLen >= this.MIN_MATCH_LENGTH && matchLen > bestLength) {
                ...
                // Early exit if we found max possible match
    ...
    // Revolutionary PIPER compression algorithm
    compress(input, level = 6) {
        ...
        // Build optimized hash chains for entire file
        const hashChains = new Array(this.HASH_SIZE);
        for (let i = 0; i < this.HASH_SIZE; i++) {
            hashChains[i] = [];
        }

        // Index all positions (no arbitrary 65KB limit!)
        for (let i = 0; i <= data.length - 3; i++) {
            const h = this.hash(data, i);
            hashChains[h].push(i);
        }

        // Compress data with PIPER algorithm
        let pos = 0;
    ...
    // PIPER decompression function with safety checks
    decompress(input) {
        const data = input instanceof Uint8Array ? input : new Uint8Array(input);
