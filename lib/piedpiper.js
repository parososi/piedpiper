...
class PiedPiperCompressor {
    constructor() {
        ...
        // PIPER Algorithm Constants
        this.WINDOW_SIZE = 32768;        // 32KB sliding window
        this.MAX_MATCH_LENGTH = 258;     // Maximum match length
        this.MIN_MATCH_LENGTH = 4;       // Minimum match for better compression
        this.HASH_SIZE = 65536;          // 64K hash table entries
        this.MAX_CHAIN_LENGTH = 128;     // Reduced for better performance
        this.CHUNK_SIZE = 64 * 1024 * 1024; // Process 64MB chunks to avoid memory issues
        this.STREAM_THRESHOLD = 100 * 1024 * 1024; // 100MB - use streaming above this
    ...
    // Optimized hash function for PIPER algorithm
    hash(data, pos) {
    ...
    // Advanced PIPER match finding with optimized hash chains and typed arrays
    findBestMatch(data, pos, hashTable, hashChains) {
        ...
        for (let i = chain.length - 1; i >= startIdx; i--) {
            ...
            // Quick rejection: verify first byte matches
            if (data[matchPos] !== firstByte) continue;

            // Quick check: also verify byte at current best length
            if (bestLength > 0 && data[matchPos + bestLength] !== data[pos + bestLength]) {
                continue;
            }

            // Count matching bytes
            let matchLen = 1; // First byte already matched
            ...
            if (matchLen >= this.MIN_MATCH_LENGTH && matchLen > bestLength) {
                ...
                // Early exit if we found max possible match or good enough match
                if (matchLen === maxMatch || matchLen > 128) break;
            }
        }
    ...
    compress(input, level = 6) {
        ...
        // Compress data with PIPER algorithm and progress reporting
        ...
        while (pos < data.length) {
            ...
            if (match.length >= this.MIN_MATCH_LENGTH) {
                ...
                writeBits(match.offset - 1, 15);  // offset (1-32768)
                writeBits(match.length - this.MIN_MATCH_LENGTH, 8);  // length beyond minimum
                pos += match.length;
            } else {
        ...
        if (version >= 3) {
            ...
            while (outPos < uncompressedSize) {
                ...
                if (flag === 0) {
                    ...
                    // Match (11)
                    const offset = readBits(15) + 1;
                    const length = readBits(8) + this.MIN_MATCH_LENGTH;
